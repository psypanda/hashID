...
function IdentifyHash($str)
{
  ...
  if(preg_match('/^\$H\$9[a-z0-9\/\.]{30}$/i', $str)) {
    array_push($possibleHashes,'MD5(phpBB3)');
  }
  if(preg_match('/^\$P\$[a-z0-9\/\.]{31}$/i', $str)) {
  ...
  if(preg_match('/^[a-f0-9]{32}:[a-z0-9]{32}$/i', $str)) {
    array_push($possibleHashes,'MD5(Joomla)');
  }
  if(preg_match('/^[a-f0-9]{32}:.{5}$/i', $str)) {
    array_push($possibleHashes,'MD5(IP.Board)');
  }
  if(preg_match('/^[a-f-0-9]{32}:[a-z0-9]{8}$/i', $str)) {
    array_push($possibleHashes,'MD5(MyBB)');
  }
  ...
  if(preg_match('/^[a-f0-9]{51}$/i', $str)) {
    array_push($possibleHashes,'MD5(Palshop)');
  }
  if(preg_match('/^\$S\$[a-z0-9\/\.]{52}$/i', $str)) {
    array_push($possibleHashes,'SHA-512(Drupal)');
  }
  if(preg_match('/^\$2a\$[0-9]{0,2}?\$[a-z0-9\/\.]{53}$/i', $str)) {
